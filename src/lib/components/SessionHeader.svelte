<script lang="ts">
    ...
    const { status, threadId, sandbox, onSandboxChange }: Props = $props();
...
<header class="session-header">
    <div class="session-header-inner">
        ...
        >
            ...
            {:else}
                <span class="status-icon" style:color={config.color} aria-label={config.label}>
                    {config.icon}
                </span>
            {/if}
        </button>
        ...
        {/if}
        <button type="button" class="header-btn" onclick={() => theme.cycle()} title="Theme: {theme.current}">
            {themeIcons[theme.current]}
        </button>
        <button type="button" class="header-btn" onclick={() => auth.signOut()} title="Sign out"> ⏻ </button>
    </div>
</header>
